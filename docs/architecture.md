...
远程中间层覆盖 profile、资料库、处理、源、授权、导出、配置、管理、维护和依赖任务。依赖安装/更新是服务端白名单异步任务；Go 只能提交、查询和取消，不能在客户端执行服务端命令。HTTP 矩阵同时验证成功、权限、令牌、版本、解码和服务错误。所有读取型远程 gateway 均显式传播失败，网络错误不会被伪装成空数组、零计数或健康状态；共享状态模型只在成功时替换最后有效数据。

Core、Pro 与 Go macOS 现在都直接组合 `ReadBoardDesktopMainFeatureView`。Core 中旧的 `ContentView`、源管理、数据看板、设置页和失败列表兼容实现已删除；Go 中的 `ReadBoardCoreSnapshot` 也已删除。产品仓库只保留服务宿主、远程连接、离线缓存、平台生命周期和打包资源。
