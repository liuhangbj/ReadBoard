...
🔗 项目地址：https://github.com/liuhangbj/ReadBoard
...
ReadBoard 不是阅读器，是信息加工厂。一个中台，替你做这六件事：

1. **统一处理方案** —— RSS / 播客 / YouTube，不同入库类型同一套流程
2. **全文获取** —— 用 defuddle 提取全文，能力与 Obsidian Web Clipper 一致
3. **LLM 评分·摘要·翻译 + 本地转录** —— 产出高质量文稿
4. **自动翻译上屏** —— 所有语言对你都是母语
5. **阅读器内嵌播放器** —— 音视频体验不输 RSS 阅读器
6. **深度匹配 Obsidian** —— 文档按导出规则直接进入知识库

SQLite + FTS5 全文搜索，零云端依赖，无账号无遥测。macOS 14+ 原生应用，MIT 开源。
