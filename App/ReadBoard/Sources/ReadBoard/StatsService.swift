...
final class StatsService: @unchecked Sendable {
    ...
    func overview() -> StatsOverview {
        ...
        s.totalContent = int("SELECT COUNT(*) FROM content")
        s.unreadCount = int("SELECT COUNT(*) FROM content WHERE read_at IS NULL AND is_archived = 0")
        s.starredCount = int("SELECT COUNT(*) FROM content WHERE starred = 1")
        s.archivedCount = int("SELECT COUNT(*) FROM content WHERE is_archived = 1")
        s.duplicateCount = int("SELECT COUNT(*) FROM content WHERE is_duplicate = 1")
        s.withFulltext = int("SELECT COUNT(*) FROM content WHERE content_md IS NOT NULL AND content_md != ''")
        s.scored = int("SELECT COUNT(*) FROM content WHERE llm_score IS NOT NULL")
        s.translated = int("SELECT COUNT(*) FROM content WHERE llm_translated_md IS NOT NULL AND llm_translated_md != ''")
        s.summarized = int("SELECT COUNT(*) FROM content WHERE llm_summary IS NOT NULL AND llm_summary != ''")
